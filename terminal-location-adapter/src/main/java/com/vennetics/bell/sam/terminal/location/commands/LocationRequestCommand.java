...
public class LocationRequestCommand extends HystrixCommand<ResponseEntity<LocationResponse>> {
    ...
    protected ResponseEntity<LocationResponse> run() {
        ...
        if (url != null) {
            final ResponseEntity<LocationResponse> response = template.exchange(buildUrl(),
                                                                                HttpMethod.GET,
                                                                                buildEntity(),
    ...
    private String buildUrl() {
        if (map.getFirst("imsi") != null) {
            return String.format("http://%s/outbound/location/requests?imsi=%s",
                                 internalServiceName,
                                 map.getFirst("imsi"));
        } else if (map.getFirst("msisdn") != null) {
            return String.format("http://%s/outbound/location/requests?msisdn=%s",
                                 internalServiceName,
                                 map.getFirst("msisdn"));
        } else {
            return null;
