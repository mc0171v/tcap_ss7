...
public class TerminalStatusRequestCommand extends HystrixCommand<ResponseEntity<StatusResponse>> {
     ...
     * @param map
     *   The parameters that constitute the status request.
     * @param template
     *   The {@link RestTemplate} to be used to address the service.
     */
    public TerminalStatusRequestCommand(final String internalServiceName,
        ...
        super(Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey(internalServiceName))
                    .andCommandKey(HystrixCommandKey.Factory.asKey("StatusRequest"))
    ...
    private String buildUrl() {
        if (map.getFirst("imsi") != null) {
            return String.format("http://%s/outbound/status/requests?imsi=%s",
                                 internalServiceName,
                                 map.getFirst("imsi"));
        } else if (map.getFirst("msisdn") != null) {
            return String.format("http://%s/outbound/status/requests?msisdn=%s",
                                 internalServiceName,
                                 map.getFirst("msisdn"));
        } else {
            return null;
